...
RATE_LIMIT_MAX=120
PROVIDER_RATE_LIMIT_WINDOW_MS=60000
PROVIDER_RATE_LIMIT_MAX=40
INVITE_CODE_SECRET=...
...
- `API_ACCESS_KEY=... ./scripts/smoke-local.sh` (adds `X-API-Key` header for protected `/api/*` routes)
...
Create an invite document in Firestore (`invites/{inviteHash}`):
...
```bash
# Custom code
npm run invite:create -- --code INV-ALPHA123

# One-time invite (default), expiring in 7 days (default)
npm run invite:create -- --max-uses 1 --expires-days 7

# Non-expiring invite
npm run invite:create -- --expires-days 0

# Replace existing invite hash doc
npm run invite:create -- --code INV-ALPHA123 --overwrite
```

Required env vars for the script:

- `FIREBASE_SERVICE_ACCOUNT_JSON` (base64-encoded service account JSON)
- `INVITE_CODE_SECRET` (recommended; keeps invite hashing stable and secret-backed)
...
- `POST /api/compare`
  - Request:
    ...
    - `imageUrl` (optional string, supports `https://...` and `data:image/...;base64,...`)
  ...
  - Behavior:
    ...
    - Remote image fetches are validated as image content and capped at 10MB before provider calls.

Invite-only signup:

- `POST /api/signup-with-invite`
  - Request:
  ...
  - Notes:
    - Invite codes are normalized and hashed server-side.
    - For one-time invites, store invite docs with `maxUses: 1`.
    - Route is protected by the same API access key middleware as other `/api/*` protected routes.

Suggested Firestore invite document shape (`invites/{inviteHash}`):
...
- API key middleware for LLM-backed routes:
  - `/api/compare`
  - `/api/openai-*`, `/api/deepseek-*`, `/api/anthropic-*`
  - Health/debug endpoints remain accessible without API access key.
  - Set `API_ACCESS_KEYS` to a comma-separated list of allowed keys.
  - Clients must send `X-API-Key` or `Authorization: Bearer <token>`.
- Rate limiting:
  - Global `/api/*` limiter via `RATE_LIMIT_WINDOW_MS` + `RATE_LIMIT_MAX`.
  - Stricter provider/compare limiter via `PROVIDER_RATE_LIMIT_WINDOW_MS` + `PROVIDER_RATE_LIMIT_MAX`.
  - Exceeded limits return `429` with `Retry-After` and rate-limit headers.
- Request body limit:
  - `JSON_BODY_LIMIT` controls maximum JSON body size (default `1mb`).
