...
const apiAccessKeys = parseAccessKeys(process.env.API_ACCESS_KEYS);
if (apiAccessKeys.length === 0) {
  logger.warn(
    'API access key guard is disabled. Set API_ACCESS_KEYS to enforce client authentication.',
  );
}
const requireApiAccess = createApiAccessMiddleware({
  keys: apiAccessKeys,
  logger,
});
const apiRateLimiter = createIpRateLimiter({
  windowMs: process.env.RATE_LIMIT_WINDOW_MS || 60_000,
  maxRequests: process.env.RATE_LIMIT_MAX || 120,
  logger,
  label: 'api',
});
const providerRateLimiter = createIpRateLimiter({
  windowMs: process.env.PROVIDER_RATE_LIMIT_WINDOW_MS || 60_000,
  maxRequests: process.env.PROVIDER_RATE_LIMIT_MAX || 40,
  logger,
  label: 'provider',
});
...
const {
  ...
  db,
  firebaseAdmin,
  inviteSecret: process.env.INVITE_CODE_SECRET || '',
});
