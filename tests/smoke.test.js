...
let authCreateUserMock = async ({ email }) => ({ uid: 'uid-default', email });
let authDeleteUserMock = async () => {};
const deletedUserIds = [];
const firestoreDocs = new Map();

const docMapKey = (collectionName, docId) => `${collectionName}::${docId}`;
const getStoredDoc = (collectionName, docId) =>
  firestoreDocs.get(docMapKey(collectionName, docId));
const setStoredDoc = (collectionName, docId, payload, options = {}) => {
  const key = docMapKey(collectionName, docId);
  const previous = firestoreDocs.get(key) || {};
  firestoreDocs.set(key, options.merge ? { ...previous, ...payload } : payload);
};
...
const mockAxios = {
  ...
  get: async () => {
    throw new Error('mockAxios.get not configured for this test');
  },
};
...
const mockFirebaseAdmin = {
  ...
  firestore: () => ({
    collection: (collectionName) => ({
      doc: (docId) => ({
        ...
        id: docId,
        get: async () => {
          if (collectionName === 'transcripts') {
            return firestoreGetMock(docId);
          }
          const stored = getStoredDoc(collectionName, docId);
          return {
        ...
        set: async (payload, options) => {
          setStoredDoc(collectionName, docId, payload, options);
        },
      }),
    }),
    runTransaction: async (runner) => {
      const tx = {
        get: async (docRef) => {
          const stored = getStoredDoc(docRef.collectionName, docRef.id);
          return {
            exists: stored !== undefined,
            data: () => stored || {},
          };
        ...
        set: (docRef, payload, options) => {
          setStoredDoc(docRef.collectionName, docRef.id, payload, options);
        },
      };
      return runner(tx);
    },
  ...
  auth: () => ({
    createUser: (...args) => authCreateUserMock(...args),
    deleteUser: async (uid) => {
      deletedUserIds.push(uid);
      return authDeleteUserMock(uid);
    },
  }),
};
...
test.afterEach(() => {
  ...
  authCreateUserMock = async ({ email }) => ({ uid: 'uid-default', email });
  authDeleteUserMock = async () => {};
...
test('POST /api/openai-chat returns sanitized provider failure in production', async () => {
  ...
  assert.deepEqual(res.body, {
    ...
    details: 'upstream internal detail',
  });
});
...
test('POST /api/signup-with-invite creates a user and consumes invite', async () => {
  const inviteSecret = 'test-secret';
  const inviteHash = hashInviteCode('INV-ALPHA', inviteSecret);
  setStoredDoc('invites', inviteHash, {
    active: true,
    uses: 0,
    maxUses: 1,
  });

  authCreateUserMock = async ({ email }) => ({
    uid: 'uid-signup-1',
    email,
  });

  const mod = loadAppWithMocks({ env: { INVITE_CODE_SECRET: inviteSecret } });
  const { signupWithInviteHandler } = mod.testHandlers;
  const res = await invokeHandlers([signupWithInviteHandler], {
    method: 'POST',
    path: '/api/signup-with-invite',
    url: '/api/signup-with-invite',
    body: {
  ...
  assert.equal(res.body.uid, 'uid-signup-1');

  const storedInvite = getStoredDoc('invites', inviteHash);
  assert.equal(storedInvite.uses, 1);
  assert.equal(storedInvite.lastUsedByUid, 'uid-signup-1');
  assert.equal(storedInvite.lastUsedByEmail, 'user@example.com');
});

test('POST /api/signup-with-invite returns 400 when payload is incomplete', async () => {
  const mod = loadAppWithMocks();
  const { signupWithInviteHandler } = mod.testHandlers;
  const res = await invokeHandlers([signupWithInviteHandler], {
    method: 'POST',
...
test('POST /api/signup-with-invite rolls back created user when invite is already used', async () => {
  const inviteSecret = 'test-secret';
  const inviteHash = hashInviteCode('INV-USED', inviteSecret);
  setStoredDoc('invites', inviteHash, {
    active: true,
    uses: 1,
    maxUses: 1,
  ...
  assert.deepEqual(deletedUserIds, ['uid-rollback-1']);
});
...
test('POST /api/compare forwards imageUrl to OpenAI responses payload', async () => {
  ...
  assert.equal(capturedPayload.input[0].content[0].type, 'input_text');
  assert.equal(
    capturedPayload.input[0].content[0].text,
    'Describe this image briefly',
  );
  assert.equal(capturedPayload.input[0].content[1].type, 'input_image');
  assert.match(
    capturedPayload.input[0].content[1].image_url,
    /^data:image\/png;base64,/,
  );
});

test('POST /api/compare accepts image data URLs without remote fetch', async () => {
  let imageFetchCalls = 0;
  mockAxios.get = async () => {
    imageFetchCalls += 1;
    return {
      data: Buffer.from('unexpected-fetch'),
      headers: { 'content-type': 'image/png' },
    };
  };
  mockAxios.post = async (url, payload) => ({
    data: {
      output: [
        {
          content: [{ type: 'output_text', text: 'Data URL summary' }],
        },
      ],
  ...
  const res = await invokeHandlers([compareHandler], {
    ...
    url: '/api/compare',
    body: {
      prompt: 'Describe this inline image briefly',
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.results[0].status, 'success');
  assert.equal(res.body.results[0].text, 'Data URL summary');
...
test('API access middleware returns 401 when key is missing', async () => {
  const mod = loadAppWithMocks({ env: { API_ACCESS_KEYS: 'alpha-key' } });
  const { requireApiAccess } = mod.testHandlers;
  const res = await invokeHandlers([requireApiAccess], {
    method: 'POST',
    path: '/api/openai-chat',
    url: '/api/openai-chat',
    body: {},
...
test('API access middleware accepts valid key', async () => {
  const mod = loadAppWithMocks({ env: { API_ACCESS_KEYS: 'alpha-key' } });
  const { requireApiAccess } = mod.testHandlers;
  const res = await invokeHandlers(
    [
...
test('API rate limiter returns 429 after configured limit', async () => {
  const mod = loadAppWithMocks({
    env: {
      RATE_LIMIT_MAX: '1',
  ...
  const { apiRateLimiter } = mod.testHandlers;
  const req = {
    method: 'POST',
    path: '/api/openai-chat',
    url: '/api/openai-chat',
  ...
  const first = await invokeHandlers(
    [
      apiRateLimiter,
      (request, response) => {
        response.status(200).json({ ok: true });
  ...
  const second = await invokeHandlers([apiRateLimiter], req);
